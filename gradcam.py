...
from gradcam_utils import get_heatmap
...
# We'll use resnet18 for visualization
...
def visualize(net, img_path, conv_layer_name):
    ...
    vizs = get_heatmap(net, image, orig_img, conv_layer_name)
    return (pred_str, (orig_img, *vizs))

# Create Grad-CAM visualization for the user provided image
cat, vizs = visualize(network, args.image_path, last_conv_layer_name)

# Write the visualiations into file
img_name = os.path.split(args.image_path)[1].split('.')[0]
suffixes = ['orig', 'gradcam_heatmap', 'gradcam_heatmap_on_img']
image_desc = ['Original Image', 'Grad-CAM heatmap ', 'Guided Grad-CAM']
